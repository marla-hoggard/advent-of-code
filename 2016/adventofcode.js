...
function getCheckSum(string) {
	...
	return countsArray.map(arr => arr[0]).slice(0, 5).join("");
}
...
function findNorthPole(data) {
	const northpole = data.find(room => {
		...
		return getCheckSum(name) === checksum &&
			shift(name, id).includes('northpole');
	});
...
function shift(string, val) {
	return string.split('').map(letter => {
		...
		return shifted <= 'z'.charCodeAt(0) ?
			String.fromCharCode(shifted) : String.fromCharCode(shifted - 26);
	}).join('');
}
...
function repititionCode(array) {
	let freq = Array(array[0].length).fill(null).map(cell => cell = {});
	array.forEach(str => {
...
function tinyScreen(directions) {
	let screen = Array(6).fill(null).map(row => Array(50).fill(0));
	directions.forEach(dir => {
		if (dir.includes('rect')) {
			const [x, y] = dir.split(/[\sx]/g).slice(1).map(el => +el);
			for (var i = 0; i < x; i++) {
				for (var j = 0; j < y; j++) {
					screen[j][i] = 1;
				}
			...
			const [row, amt] = dir.split(/=|\sby\s/g).slice(1).map(el => +el);
			screen[row] = screen[row].map((el, i, r) => r[(i + 50 - amt) % 50]);
		}
		else { // column
			const [col, amount] = dir.split(/=|\sby\s/g).slice(1).map(el => +el);
			screen = screen.map((row, r, screen) => {
				return screen[r].slice(0, col).concat(screen[(r + 6 - amount) % 6][col], screen[r].slice(col + 1));
			})
		}
	});
	console.log(screen);
	return screen.map(row => row.reduce((prev, cur) => prev + cur)).reduce((prev, cur) => prev + cur);
}
